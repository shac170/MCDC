...
def update_weight_windows(data, mcdc):
    ...
    if epsilon[WW_WOLLABER1] > 0:
        ...
        mcdc["technique"]["ww"]["center"][idx_census + 1] = (mcdc["technique"]["ww"]["center"][idx_census + 1]) * (
            1
            + (1 / epsilon[WW_WOLLABER1] - 1)
            * np.exp(-(mcdc["technique"]["ww"]["center"][idx_census + 1] - w_min) / epsilon[WW_WOLLABER1])
        )
    if epsilon[WW_MIN] > 0:
        mcdc["technique"]["ww"]["center"][idx_census + 1] = mcdc["technique"]["ww"]["center"][idx_census + 1] * (1 - epsilon[WW_MIN]) + epsilon[WW_MIN]
        mcdc["technique"]["ww"]["center"][idx_census + 1][mcdc["technique"]["ww"]["center"][idx_census + 1] <= 0] = epsilon[WW_MIN]
    mcdc["technique"]["ww"]["center"][idx_census + 1] /= np.max(mcdc["technique"]["ww"]["center"][idx_census + 1])
    ...
    f.create_dataset("weight_windows/center", data=mcdc["technique"]["ww"]["center"][idx_census + 1])
    f.close()
...
def ww_alpha(data, mcdc):
    ...
    with objmode(center="float64"):
        ...
        alpha = (1 / dt) * np.log(abs(flux1 / flux2) + 1e-2)
        alpha[flux2 == 0] = 1 / dt
        alpha[alpha > 3] = 2
...
def ww_dmd(data, mcdc):
    ...
    with objmode(center="float64"):
        for n in range(n_snapshot):
            ...
            snapshots.append(flux)

        snapshots = np.flip(np.array(snapshots),0)
        ...
        b = np.linalg.pinv(PSI) @ snapshots[0]

        t = n_snapshot * dt 

        center = np.real(PSI @ np.diag(np.exp(omega * t)) @ b)
