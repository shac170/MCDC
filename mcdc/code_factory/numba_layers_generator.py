...
def generate_numba_layers(simulation):
    ...
    N = {}
    for name in bank_names:
        bank_name = name.removeprefix("bank_")
        N[f"{bank_name}_particle"] = int(getattr(simulation, name).size[0])
    ...
    simulation_dtype = type_.make_simulation_type(N)
...
def build_structures():
    """Build the static dtype structures directly from the object annotations."""
    annotations = {}
    structures = {}
    accessor_targets = {}
    ...
    # Simulation depends on all other structures, so build it last.
    annotations["simulation"] = annotations.pop("simulation")
    structures["simulation"] = structures.pop("simulation")
    accessor_targets["simulation"] = accessor_targets.pop("simulation")
    ...
    for mcdc_class in mcdc_classes:
        ...
        for class_ in classes:
            new_annotations = {
                ...
                if key not in ["label", "non_numba"]
                and ("non_numba" not in dir(class_) or key not in class_.non_numba)
            }
            if new_annotations and isinstance(
                next(iter(new_annotations.values())), str
            ):
                new_annotations = parse_annotations_dict(new_annotations)
            annotations[mcdc_class.label].update(new_annotations)
    ...
    for name in bank_names:
        annotations[name] = {
            key: value
            for key, value in ParticleBank.__annotations__.items()
            if key not in ["label", "non_numba"]
            and (
                "non_numba" not in dir(ParticleBank)
                or key not in ParticleBank.non_numba
            )
        }

    simulation_object_structure = []
    for field, hint in annotations["simulation"].items():
        hint_origin = get_origin(hint)
        hint_args = get_args(hint)
        if hint in all_classes:
            simulation_object_structure.append((field, hint))
        elif hint_origin == list and hint_args[0] in all_classes:
            simulation_object_structure.append((field, list, hint_args[0]))
    ...
    # The generated bank and collection setters substitute their actual runtime sizes.
    placeholder_size = (1,)
    for name in bank_names:
        structures[name].append(
            (
                "particle_data",
                into_dtype(structures["particle_data"]),
                placeholder_size,
            )
        )
        structures["simulation"] = [(name, into_dtype(structures[name]))] + structures[
    ...
    for item in simulation_object_structure:
        ...
        if item[1] == list:
            base_class = item[2]
            if base_class not in polymorphic_bases:
                collection_structures.append(
                    (
                        field,
                        into_dtype(structures[base_class.label]),
                        placeholder_size,
                    )
                )
                collection_structures.append(
                    (f"N_{plural_to_singular(field)}", type_map[int])
                )
            else:
                for class_ in mcdc_classes:
                    if issubclass(class_, base_class):
                        ...
                        collection_structures.append(
                            (f"N_{class_.label}", type_map[int])
                        )
        elif item[1] in mcdc_classes and issubclass(item[1], MCDCBase):
            collection_structures.append((field, into_dtype(structures[item[1].label])))

    structures["simulation"] = collection_structures + structures["simulation"]
    return structures, accessor_targets, structure_order


def generate_numba_types(structures, structure_order):
    text = "# The following is automatically generated by rebuild_numba_support.py\n\n"
    ...
    for item in structures[bank_names[0]]:
        if item[0] == "particle_data":
            text += "        ('particle_data', particle_data, (particle_capacity,)),\n"
        else:
            text += decode_structure_item(item, "    ")
    text += "    ])\n\n"
    ...
    text += "    return into_dtype([\n"
    for item in structures["simulation"]:
        if item[0] in bank_names:
            bank_name = item[0].removeprefix("bank_")
            text += (
                f"        ('{item[0]}', "
                f"make_bank_type(N['{bank_name}_particle'])),\n"
            )
        elif type(item[1]) == np.dtypes.VoidDType and len(item) == 3:
            singular_field = plural_to_singular(item[0])
            text += (
                f"        ('{item[0]}', {singular_field}, "
                f"(N['{singular_field}'])),\n"
            )
        else:
            text += decode_structure_item(item, "    ")
    ...
    (Path(mcdc.__file__).parent / "numba_types.py").write_text(text, encoding="utf-8")


def rebuild_numba_support():
    structures, accessor_targets, structure_order = build_structures()
