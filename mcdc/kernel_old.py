...
# Hybrid functions
from LOSM import losm_timestep
...
@njit
def score_second_moment(s, g, t, x, y, z, flux, P, score):
    ux = P["ux"]
    uy = P["uy"]
    uz = P["uz"]
...
def ww_auto(mcdc):
    ...
    if np.max(np.abs(flux))>0:
        ...
        if mcdc["technique"]["ww_auto"] == 1:
            ...
            ax[0].plot(new_flux, label='Alpha')
            ax[0].plot(phi_ref, label='Reference')
            #ax[0].plot(old_ref, label='Reference previous')
            ax[0].plot(flux, label='Previous')
            ax[0].plot(old_flux, label='n-2 Flux')
            ax[0].set_yscale('log')
            ax[0].legend()
            ax[0].set_title('Fluxes at step '+str(idx_census-1))

            # Plot the second subplot
            ax[1].plot(new_flux/np.max(new_flux)*(1-epsilon)+epsilon, label='Alpha')
            ax[1].plot(phi_ref*(1-epsilon)+epsilon, label='Reference')
            ax[1].plot(flux/np.max(flux)*(1-epsilon)+epsilon, label='Previous')
            #ax[0].set_yscale('log')
            ax[1].legend()
            ax[1].set_title('Weight Windows at step '+str(idx_census-1))

            # Plot the third subplot
            ax[2].plot(alpha, label='Alpha')
            #ax[1].set_yscale('log')
            ax[2].legend()
            ax[2].set_title('alpha'+str(idx_census-1))

            fig.tight_layout()
            #plt.show()
            plt.savefig('windows/'+str(idx_census)+'.png')
            '''

        #import matplotlib.pyplot as plt
        #plt.clf()
        #plt.plot(new_flux,label='+1')
        #plt.plot(old_flux,label='-1')
        #plt.plot(flux,label='0')
        #plt.plot(alpha,label='alpha')
        #plt.plot(mcdc["technique"]["ww"][idx_census-1,:,0,0],label='window')
        #print(np.max(mcdc["technique"]["ww"][idx_census-1,:,0,0]))
        #plt.plot( new_flux/np.max(new_flux),label='newflux')
        #plt.yscale('log')
        #plt.legend()
        #plt.show()
        #mcdc["technique"]["ww"][idx_census-1,:,0,0] = new_flux/np.max(new_flux)
